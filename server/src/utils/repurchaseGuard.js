...
// 기본값은 본계정/활성 reviewer_identity만 신뢰한다. 로그인한 리뷰어의 명의 선택 UI처럼
// allowDeclaredAccounts=true를 명시한 경로에서는 그 소유자 DB에 등록된 타계정도 조회한다.
// 단, 다른 리뷰어의 본계정 또는 활성 reviewer_identity와 겹치는 번호는 제외한다. 타 소유자가
// 편집 가능한 sub_accounts에 같은 번호를 적었다는 이유만으로 정당한 명의를 막지는 않는다.
// 명의 확인에 SMS를 사용하지 않는 제품 결정 때문에 "미등록 타인의 번호"까지 증명할 수는 없으며,
// 실제 참여 허용의 최종 권위는 apply의 phone8 충돌검사 + 재참여 하드 가드다.
async function checkRepurchaseStatusForAccounts(dbOrClient, {
  campaignIds, phone8List, ownerPhone8, ownerReviewerId, allowDeclaredAccounts = false,
} = {}) {
  const out = new Map();
  const ids = Array.from(new Set((campaignIds || []).map(String).filter(Boolean)));
  ...
  const allowDeclared = allowDeclaredAccounts === true && !!ownerId;
  if (repurchaseDays() <= 0 || !ids.length || !phones.length) return out;
  const { rows } = await dbOrClient.query(
    `WITH requested_phones AS (
       SELECT requested.phone8,
              (requested.phone8 = $3 OR EXISTS (
                SELECT 1 FROM reviewer_identities own_ri
                 WHERE own_ri.owner_reviewer_id = $4::uuid
                   AND own_ri.current_phone8 = requested.phone8
                   AND own_ri.status = 'active'
              )) AS ownership_verified,
              EXISTS (
                SELECT 1 FROM reviewers other
                 WHERE other.id <> $4::uuid
                   AND (other.phone8 = requested.phone8 OR EXISTS (
                     SELECT 1 FROM reviewer_identities foreign_ri
                      WHERE foreign_ri.owner_reviewer_id = other.id
                        AND foreign_ri.current_phone8 = requested.phone8
                        AND foreign_ri.status = 'active'
                   ))
              ) AS externally_claimed
         FROM unnest($2::text[]) AS requested(phone8)
     ), trusted_phones AS (
       SELECT phone8, ownership_verified
         FROM requested_phones
        WHERE ownership_verified OR ($5::boolean AND NOT externally_claimed)
     ), scoped_campaigns AS (
       SELECT id AS campaign_id, repurchase_days
       ...
       SELECT rc.id AS campaign_id, rc.repurchase_days, ca.phone8, ca.submitted_at
        ...
        WHERE rc.id = ANY($1::text[])
          ...
          AND (ca.order_submission_id IS NULL OR EXISTS (
           ...
           AND ca.phone8 IN (SELECT phone8 FROM trusted_phones)
      )
     SELECT scoped.campaign_id, scoped.repurchase_days, trusted.phone8,
            trusted.ownership_verified, MAX(history.submitted_at) AS last_submitted_at
       FROM scoped_campaigns scoped
       CROSS JOIN trusted_phones trusted
       LEFT JOIN campaign_history history
         ON history.campaign_id = scoped.campaign_id AND history.phone8 = trusted.phone8
       GROUP BY scoped.campaign_id, scoped.repurchase_days, trusted.phone8,
                trusted.ownership_verified`, [ids, phones, owner, ownerId, allowDeclared]);
  const now = Date.now();
  for (const r of rows) {
    const days = repurchaseDays(r.repurchase_days);
    if (days <= 0) continue;
    ...
    if (!r.last_submitted_at && !r.last_at) {
      out.get(p8).set(String(r.campaign_id), {
        status: 'ready', days, history: 'none', ownershipVerified: r.ownership_verified === true,
      });
      continue;
    }
    ...
    out.get(p8).set(String(r.campaign_id), {
      status, days, history: 'found', lastSubmittedAt: at, availableFrom,
