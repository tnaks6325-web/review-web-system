 ...
 * ══════════════════════════════════════════════════════════════
 * 배경(2026-08 사용자 확정): **리뷰웹시스템[3버전]이 메인**이고 관리자 대시보드는 폐기 대상이다.
 * 없어질 화면에서 난 오류가 작업보드 「시스템 오류로그」를 채우면 정작 손봐야 할 게 묻히므로
 * 목록에서 제외한다.
 *
 ...
 *   리뷰웹시스템[3버전]이 직접 호출하는 로그인 경로다(workdesk.html 의 로그인 폴백 4단). 통째로
 *   걸러내면 **작업보드 로그인·인트라넷 SSO 장애가 어느 화면에도 안 뜨는 무신호 상태**가 된다.
 *   (막으려던 것보다 큰 손실 — 캡처 검수 fail-open·옵션 감사 경고전용과 같은 판단.)
 *
 * ★ 공용 인프라 오류(시트·Drive·AI·DB·큐·리뷰어 제출 등)는 flow 가 admin 이 아니므로
 *   그대로 남는다 — 관리자 대시보드에서 눌러 났든 작업보드에서 눌러 났든 같은 기능의 같은
 *   고장이라 가려내면 손해다.
 *
...
/**
 * `/api/admin/*` 이지만 **리뷰웹시스템[3버전]이 쓰는** 경로 — 제외 대상에서 뺀다.
...
function adminUiWhereSql(params, negate) {
  ...
  // "관리자 대시보드에서 난 오류" = flow=admin AND 경로가 작업보드 경로 목록에 없음
