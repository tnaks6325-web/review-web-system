...
/** 목록 카드용 유입방식 일괄 조회. 공고별 연결 작업오더를 우선하고, 보조 원본 오더를 폴백으로 쓴다. */
async function _fetchInflowTypesForCampaigns(rows) {
  const camps = (rows || []).filter(r => r && r.id);
  if (!camps.length) return new Map();
  try {
    const ids = camps.map(r => String(r.id));
    const sourceIds = camps.map(r => String(r.source_work_order_id || '')).filter(Boolean);
    const { rows: orders } = await pool.query(
      `SELECT id, linked_campaign_id, inflow_type, updated_at
         FROM work_orders
        WHERE linked_campaign_id = ANY($1::text[])
           OR id = ANY($2::text[])
    ...
    for (const o of orders) {
      const linkedId = String(o.linked_campaign_id || '');
      if (linkedId && ids.includes(linkedId) && !out.has(linkedId)) out.set(linkedId, o.inflow_type || '');
    }
    for (const c of camps) {
      if (out.has(c.id)) continue;
      const sourceId = String(c.source_work_order_id || '');
      const match = sourceId && orders.find(o => String(o.id) === sourceId);
      if (match) out.set(c.id, match.inflow_type || '');
    }
    return out;
  } catch (_) {
    return new Map();
...
/** 참여형 활성화 게이트: 시간창과 일일 진행 건수만 확인한다.
 *  시트·탭 연결은 작업오더 연동을 위한 선택 정보이므로 게시를 막지 않는다. */
...
router.get('/list', async (req, res, next) => {
  try {
    await _ensureTables();
    const now = new Date();
    let { rows, countsMap, optionsMap, feeMap, crMap, filledMap, inflowMap } = _listCache;
    if (!rows || now.getTime() - _listCache.at > LIST_CACHE_MS) {
      const q = await pool.query(`
        SELECT id, title, channel, channel_custom, manager, time_range,
               ...
               carry_mode, skip_weekends, cash_receipt_required, source_work_order_id  -- ★ 098(코드리뷰 B1): dailyQuota 가 읽는다 — 빠지면 목록은 자동 이월
      ...
      inflowMap = await _fetchInflowTypesForCampaigns(rows);
      ...
      _listCache = { at: now.getTime(), rows, countsMap, optionsMap, feeMap, crMap, filledMap, inflowMap };
    }
    ...
    const data = rows.map(r => {
      ...
      view.inflowType = inflowMap.get(r.id) || '';
...
router.get('/admin/list', authMiddleware, adminOrMasterMiddleware, async (req, res, next) => {
  try {
    ...
    try { inflowMap = await _fetchInflowTypesForCampaigns(rows); } catch (_) { /* 카드 표기는 빈 값으로 폴백 */ }
    ...
    const data = rows.map(r => {
      if (!r.participation_mode) return { ...r, inflowType: inflowMap.get(r.id) || '' };
...
router.get('/admin/:id/preview', authMiddleware, adminOrMasterMiddleware, async (req, res, next) => {
  try {
    ...
    const optionViews = await _loadOptionViews(pool, id, st, now);
    // 실제 공고 상태(마감·게시 전)와 무관하게, 미리보기에서는 열려 있는 옵션을 가상으로 고른다.
    // 이 플래그는 이 읽기 전용 응답에서만 쓰이며 DB 재고·실제 리뷰어 apply 규칙은 바꾸지 않는다.
