...
async function submitExternalOrder({
  ...
  //     시트 행만 생기고 정원은 안 깎이는 어긋난 상태가 남는다).
  //   ★★ **막지 않는다 — 확인만 받는다**(사용자 확정 2026-08-19 "나"안): 외부모집은 이미 약속된
  //     구매라 하드블록하면 그 흐름이 죽는다. 대신 `allowOverDaily` 없이 오면 초과 사실과 숫자를
  ...
  if (campaignId && !allowOverDaily) {
    ...
    if (dq && dq.remaining <= 0) {
      return {
        ...
        error: `오늘 모집인원(${dq.quota}명)이 이미 찼습니다 — 현재 ${dq.todayCount}명. 초과 접수하려면 확인이 필요합니다`,
      };
    }
  }
