...
/* ★★ 시트가 실제로 쓰는 표기를 그대로 담는다 — `구매날짜` 는 '구매일' 을 포함하지 않아
 *    (구매+날짜) 지금까지 날짜 칸으로 인식되지 않았다(운영 실측 2026-08-07: 105탭 중 14탭이
 *    `구매날짜`). 그 탭들은 ① 탈시트 이관 점검이 "준비 줄을 셀 수 없음"으로 잠기고
 *    ② 시트 일정 자동 인식(063)도 배포 이래 한 번도 걸리지 않았다.
 * ★ `날짜`·`일자` 처럼 넓게 잡으면 안 된다 — `findDateColumnIndex` 는 **가장 앞선 매칭**을
 *   쓰므로 `입금일자`·`리뷰날짜` 같은 칸을 구매일로 오인해 그날 정원이 통째로 틀어진다. */
const DATE_HEADER_KEYWORDS = ['구매일', '구매날짜', '구매 날짜', '시작일', '주문일', '배정일', '진행일'];
...
async function deriveSchedules(db, tabs, now = new Date()) {
  ...
  /* ★★ 무시트 탭은 시트 일정 파생에서 뺀다(탈 구글시트 W2-b · D3-a 사용자 확정).
     ...
     일정을 파생하면 "시트가 진실원본"인 상태가 되어 **날짜별 정원이 작업표 미러를 계속 따라간다**
     (2026-08-07부터 조절 자체는 가능하지만, 조절하지 않은 날은 여전히 미러가 정한다).
     날짜별 정원의 진실원본은 **달력**
