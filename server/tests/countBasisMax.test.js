 ...
 *
 * 고정:
 *  ① 합치는 것 = 어제까지 확정 · 오늘 확정 · 이월/보류 기준선 이후 확정(구간마다 큰 값)
 ...
 *  ⑤ 실제 효과: 공고 밖 주문이 오늘 인원·남은 자리·앞날 예상에 들어간다(업소용 간장 모양 재현)
...
(async () => {
  console.log('\n[1] 합치기 — 구간마다 큰 값');
  {
    ...
    ok('오늘 = max(1, 3) = 3', o.todaySubmitted === 3);
    ...
    ok('원래 신청 수 보존', o.applications && o.applications.submittedBeforeToday === 361 && o.applications.todaySubmitted === 1);
  ...
  console.log('\n[4] 실제 효과 — 업소용 간장 모양(총 750 · 일 35 · 종료일 뒤에 붙이기)');
  {
    ...
    ok('이월도 남은 자리(750 − 404) 이하', pc === null || pc <= 750 - 404, String(pc));
  }
  ...
  if (process.env.PGTEST_URL) {
    ...
    try {
      ...
      const db = { query: (sql, p) => (/order_submissions/.test(String(sql)) ? origQ(sql, p) : Promise.resolve({ rows: [] })) };
      ...
      ok('이월 기준선 이후(어제까지) — 기준선이 없으면 0', Number.isFinite(L.ordersSinceCarry));
      ok('★ 합쳐진 값: 어제까지 3 · 오늘 3(신청 0)', o.submittedBeforeToday === 3 && o.todaySubmitted === 3 && o.countBasis === 'max', JSON.stringify(o));
    } finally {
