...
test('최근 48시간 큐 누락 복구 주문은 원장 조건을 모두 만족할 때만 허용한다', async () => {
  assert.equal(await canOverflow(db(valid()), 'os-1', { allowRecoveredQueueOverflow: true, workboardId: 'wb-1' }), true);
});

for (const [name, row, options] of [
  ['일반 큐', valid(), { workboardId: 'wb-1' }],
  ['48시간 초과', valid({ submitted_at: new Date(Date.now() - 49 * 60 * 60 * 1000) }), { allowRecoveredQueueOverflow: true, workboardId: 'wb-1' }],
  ['주문번호 없음', valid({ order_num: null }), { allowRecoveredQueueOverflow: true, workboardId: 'wb-1' }],
  ['다른 작업보드', valid(), { allowRecoveredQueueOverflow: true, workboardId: 'wb-2' }],
  ['완료 상태 재시도', valid({ mirror_status: 'written' }), { allowRecoveredQueueOverflow: true, workboardId: 'wb-1' }],
]) {
