...
(async()=>{try{
 ...
 await db.exec('ALTER TABLE recruit_campaigns ADD COLUMN linked_sheet_id text, ADD COLUMN linked_tab_name text, ADD COLUMN linked_tab_gid text; ALTER TABLE campaign_participants ADD COLUMN tab_gid text; ALTER TABLE order_submissions ADD COLUMN tab_gid text');
 ...
 await db.query("INSERT INTO index_master_archive(sheet_id,tab_name) VALUES('campaign:case11','작업11')");
 ...
 await db.query("INSERT INTO index_master_archive(sheet_id,tab_name) VALUES('legacy-sheet','legacy-tab')");
 ...
 await db.query("UPDATE index_master_archive SET tab_name='renamed-tab',tab_gid='777' WHERE sheet_id='legacy-sheet'");
 await db.query("UPDATE recruit_campaigns SET linked_tab_gid='777' WHERE id='case7'");
 assert.ok(!(await sheetless()).rows.some(r=>r.id===uid(7)),'renamed virtual board is excluded by stable gid');
 await db.query("UPDATE recruit_campaigns SET linked_tab_gid='' WHERE id='case7'");
 assert.ok((await sheetless()).rows.some(r=>r.id===uid(7)),'blank gid does not match another archived tab');
 await db.query("UPDATE recruit_campaigns SET linked_tab_gid='777',linked_sheet_id='different-sheet' WHERE id='case7'");
 assert.ok((await sheetless()).rows.some(r=>r.id===uid(7)),'same gid on another sheet is not an archive match');
