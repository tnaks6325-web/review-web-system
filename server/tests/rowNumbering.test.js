...
(async () => {
  ...
  console.log('\n[G] 라우트·화면 배선');
  {
    ...
    /* ★ 2026-08-21: [🧹 줄 정리] 를 제거했으므로 그쪽으로 보내는 안내는 **죽은 길**이다.
         검사 의미를 뒤집어 고정한다 — 없는 버튼을 가리키지 않고, 이 정리가 직접 한다고 말한다. */
    ok('★ 제거된 [🧹 줄 정리] 로 보내지 않는다(죽은 안내 금지)', !/줄 정리\]를 쓰세요/.test(fe));
    ok('★ 짝 빈 줄은 이 정리가 함께 내린다고 말한다', /짝이 없는 빈 자리는 이 정리가 함께 내립니다/.test(fe));
