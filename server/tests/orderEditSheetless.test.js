/**
 * orderEditSheetless.test.js — 관리자 주문 편집(order-edit)의 무시트 반영 회귀가드 (2026-08-21)
 *
 ...
 *
 * 고정하는 것(diag.routes order-edit 핸들러 본문):
...
// ★ 함수 본문으로 잘라서 본다(파일 전체 검사는 다른 라우트의 유사 문자열이 대신 통과시킨다).
const start = src.indexOf("router.post('/order-edit'");
assert.ok(start >= 0, 'order-edit 라우트 존재');
const end = src.indexOf("router.post(", start + 10);
const body = src.slice(start, end > start ? end : undefined);

// ① 무시트 판정 단일 출처
assert.ok(/require\('\.\.\/utils\/sheetlessScope'\)\.isSheetless\(pool, os\.sheet_id, os\.tab_name\)/.test(body),
...
assert.ok(/require\('\.\.\/services\/sheetlessOrder\.service'\)\.writeOrderToWorktable\(\{/.test(body),
...
assert.ok(/enqueue\('order_update', \{ orderSubmissionId, editSeq, edits: clean \}\)/.test(body),
  '종전 enqueue 페이로드 불변');
// ④ 응답 고지
assert.ok(/sheetlessApplied: out\.sheetlessApplied \|\| null/.test(body),
  '응답이 sheetlessApplied 로 사실을 말한다');
// 예외는 fail-soft 로 폴백(라우트 500 금지 — 편집 자체는 이미 DB 확정)
assert.ok(/catch \(e\) \{ sheetlessApplied = \{ ok: false, reason: 'exception', message: e\.message \}; \}/.test(body),
  '작업표 기록 예외 = 폴백(편집 확정을 죽이지 않는다)');
// 편집 필드 화이트리스트에 date_str 존재(구매일자 교정 경로의 전제)
assert.ok(/_ORDER_LEDGER_EDIT_FIELDS = \[[^\]]*'date_str'/.test(src), 'date_str 편집 허용 필드');
