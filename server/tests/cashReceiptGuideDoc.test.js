...
ok('경고 문구가 실제와 일치(발행 신청은 결제 전에)',
  /결제 화면을 지나기 전에 꼭 발행을 신청해 주세요/.test(doc)
  && /결제 화면을 지나기 전에 꼭 발행을 신청해 주세요/.test(wd));
ok('★ D안(2026-08-05): 발행확정 0~3일 안내 + 캡처는 구매양식 제출 필수 아님 — 문서·화면 동일 문구',
  /발행이 확정된 뒤\(배송완료·구매확정 후 0~3일\)/.test(doc)
