...
function run() {
  ...
  // ── 케이스4c-2(PR 리뷰 회귀가드): 제출열이 정확히 '리뷰'인 레거시 표에서 그보다 앞선
  //   리뷰옵션·리뷰가이드는 작업지시다. 값이 항상 있어도 제출완료로 세면 안 된다.
  const v4c2 = [
    ['번호', '구매일자', '주문자', '리뷰옵션', '리뷰가이드', '리뷰', '입금'],
    ['1', '8 / 4 (화)', '미제출자', '포토리뷰', '사진 3장', '', ''],
    ['2', '8 / 4 (화)', '제출자', '텍스트', '200자 이상', '8/5 12:00', ''],
  ];
  const r4c2 = parseTabRows(v4c2, 's4c2', 'tabD3-2', 'gidD3-2', 'D3-2', KW);
  assert.equal(r4c2[0].submitCol, '리뷰', "★ 리뷰옵션·리뷰가이드보다 bare '리뷰' 제출열을 고른다");
  assert.equal(r4c2[0].isSubmitted, false, '작업지시만 있고 리뷰가 비면 미제출');
  assert.equal(r4c2[1].isSubmitted, true, 'bare 리뷰 열에 값이 있으면 제출완료');
  ...
  // ── 케이스4e(회귀가드, 2026-08-19 블로그 E2E 실측): 블로그체험단 작업표의 '포스팅제출일'
  //   헤더가 완료신호 '제출'을 포함해 2단계에서 리뷰제출 열로 오탐되던 사고. 제출 표시가
  //   날짜 칸에 들어갔다 포스팅제출일 스탬프에 덮이고 '리뷰' 칸은 비어 is_submitted 가
  //   장부 재생성마다 FALSE 로 무너졌다 → '포스팅' 계열은 제외하고 '리뷰' 열을 채택해야 한다.
  const v4e = [
    ['번호', '구매일자', '주문번호', '주문자', '수취인', 'id', '연락처', '주소', '은행', '계좌번호', '예금주', '결제금액', '리뷰', '입금', '블로그URL', '포스팅결과URL', '포스팅제출일'],
    ['1', '8 / 19 (수)', '', 'E2E블로거', 'E2E블로거', 'e2e_id', '010-9999-0819', '주소', '국민', '9990000', 'E2E블로거', '15000', '제출', '', 'https://blog.naver.com/a', 'https://blog.naver.com/a/1', '2026-08-19'],
    ['2', '8 / 19 (수)', '', '미제출자', '미제출자', 'x', '010-1111-0819', '주소', '', '', '', '', '', '', '', '', ''],
  ];
  const r4e = parseTabRows(v4e, 's4e', 'tabE', 'gidE', 'E', KW);
  assert.equal(r4e[0].submitCol, '리뷰', "★ 블로그 작업표: '포스팅제출일'이 아니라 '리뷰'가 제출열");
  assert.equal(r4e[0].isSubmitted, true, "리뷰='제출' → 제출완료");
  assert.equal(r4e[1].isSubmitted, false, "리뷰 공란 → 미제출(포스팅제출일 공란 여부와 무관)");
