...
/* ★ 넓혔을 뿐 **지우지 않았다** — 종전 정의는 (a) 갈래로 그대로 산다.
     줄 서두가 "구매확정 완료 화면, 또는 …" 으로 바뀌었으므로 접두가 아니라 (a) 본문을 본다. */
ok('purchase_confirm 종전 정의(구매확정 문구 + 완료 표시)는 (a) 로 그대로 남아 있다',
  /- "purchase_confirm": 구매확정 완료 화면/.test(gem)
  && /\(a\) "구매확정" 문구와 함께 완료됨을 나타내는 표시/.test(gem));

/* ═══ ② 넓힌 것 ═══════════════════════════════════════════════════ */
console.log('\n▶ 구매확정 완료 신호 인정(B안)');
/* ★★ 사용자 확정 2026-08-23 (나)안 — 인정 범위를 세 갈래로 명시한다.
     ⚠ (c) 는 "구매확정을 눌렀는지"를 화면으로 검증하지 않는 완화다(대가를 알고 고른 것).
        되돌리려면 (c) 를 지우고 캐시 접두를 올린다. */
ok('★ 인정 갈래를 (a)(b)(c) 로 명시한다',
  /purchase_confirm 이다: \(a\)/.test(gem) && /\(b\) 적립이 이미 완료되었다는 표시/.test(gem)
  && /\(c\) 그 주문에 대한 리뷰 작성 대기 화면/.test(gem));
ok('(a) 종전 정의(구매확정 문구 + 완료 표시) 유지', /구매확정 완료, 구매확정됨, 확정일시/.test(gem));
ok('(b) 신고 화면을 예로 못박는다(네이버페이 적립 완료)',
  /orders\.pay\.naver\.com/.test(gem) && /적립되었어요/.test(gem));
ok('★ (c) 리뷰 작성 대기 화면의 단서를 적는다(작성기한·리뷰쓰기 유도)',
  /작성기한/.test(gem) && /리뷰 쓰고 N원 받기/.test(gem));
/* ★★ 종전의 "리뷰 작성 적립은 제외" 문장은 **제거**됐다 — 등록된 구매확정 예시(2/2장)에도
     `리뷰 쓰고 최대 150원 받기` 가 있어, 그 제외가 **구제하려던 화면을 도로 배제**했다.
     되살리면 같은 자리에서 다시 밟는다. */
ok('★★ 잘못 좁히던 제외 문장이 없다(예시 2/2장을 배제하던 규칙)',
  !/리뷰 작성에 대한 적립이라고 화면에 적혀 있으면 purchase_confirm 이 아니다/.test(gem));
ok('★ order_capture 줄이 배타 조건을 말한다(두 종류가 같은 화면을 서로 가져가지 않게)',
  /위 purchase_confirm 의 \(a\)\(b\)\(c\) 중 어느 것도 보이지 않는다/.test(gem));
