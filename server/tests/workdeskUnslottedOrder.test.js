...
t('주문값은 옮기되 유효 슬롯 번호는 보존', () => {
  const row = svc.__mergeUnslottedOrderRowForTest({ 번호: '601', 담당자: '만두' }, { 번호: '', 주문자제출: '임은진', 결제금액: '78000' }, 601);
  assert.deepStrictEqual(row, { 번호: '601', 담당자: '만두', 주문자제출: '임은진', 결제금액: '78000' });
});
t('서버는 목표 안의 빈 슬롯만 잠그고 선택', () => {
...
t('관리자 확인을 요구하는 전용 경로와 화면 실행 경로가 있다', () => {
