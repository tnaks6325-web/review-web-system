...
- **부품3b 관측 대시보드 = cutover 결정 허브**: `overview()`가 완성 신호(카운트일치·소유·발주연결·`sourceOfTruth`)를 **`cutoverReady` 종합 triage**로 수렴(=시트원본 && 카운트일치 && 소유 && 발주; 최종 게이트는 정밀 `real=0`) + **write-back 건강**(participant_edits `editCount`·`held`·`blocked` 집계) 노출. 관측 뷰(workdesk.html)에 **"cutover" 종합 판정 컬럼**(✅ 준비/미충족·사유/정밀 필요/전환됨)·**"write-back" 컬럼**·헤더 "cutover 준비 N·전환됨 N" 요약. 읽기전용·무위험. 회귀가드 `tests/trackBOverviewCutover.test.js`.
