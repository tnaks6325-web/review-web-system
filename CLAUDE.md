...
- **탭별 DB CSV 다운로드(`GET /api/raw/rows.csv?sheetId&gid`)**: 그 탭에 저장된 `raw_sheet_rows`(시트 전체 미러) 전 행을 페이지네이션 없이 CSV로 내려준다(UTF-8 BOM). 관리자 **RAW 미러 데이터 뷰어(`raw-mirror.html`)** 의 "CSV 다운로드" 버튼이 인증 헤더로 fetch→blob 다운로드. 탭별 저장 데이터 백업/감사/오프라인 분석용. (구매 주문 원장은 별도로 `GET /api/diag/order-stuck-export?sheetId&tabName[&includeWritten=true]`로 추출.)
