...
- **부품4 소유 UI**(adminOrMaster): `GET /advertisers`(소유수 포함)·`/tabs`(활성탭, participants/tabs가 master전용이라 재노출)·`/ownership` GET/POST/DELETE. workdesk.html 소유지정 뷰에서 업체 선택→시트/탭 지정·해제. **＋업체 추가**(프론트→`POST /api/portal/advertisers` 재사용)로 거래처 신규 등록(담당 AE 포함) 즉시 목록 반영.
