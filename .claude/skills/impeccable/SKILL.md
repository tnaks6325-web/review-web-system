---
name: impeccable
description: 새 웹사이트·새 화면·새 컴포넌트를 디자인하거나, 화면 전체의 전면 리디자인·디자인 크리틱/오딧을 요청받았을 때 사용(인트라넷·업무 도구·대시보드 포함 — Operate 모드 보유). Design-director skill for shaping, critiquing, auditing, polishing frontend interfaces. 기존 리뷰웹시스템(workdesk/admin) 화면의 기능 수정·버그 수정·소규모 UI 조정에는 트리거하지 않는다 — 그 경우 CLAUDE.md의 레포 규율(단일 출처·회귀가드·기존 idiom)이 우선하며, 디자인 판단이 필요하면 사용자가 /impeccable 로 명시 호출한다. Not for backend-only or non-UI tasks.
version: 4.0.4
user-invocable: true
...
<!-- 설치 메모(2026-08-06, Apache 2.0 수정 고지): github.com/pbakaus/impeccable 의
     .claude/skills/impeccable 빌드(v4.0.4) 원본. 수정 = frontmatter description 만
     (이 레포에서의 트리거 기준을 새김) — 본문·reference·scripts 무수정.
     훅·settings.json 은 설치하지 않았다(자동 검출기 상시 실행 없음 — 스킬 사용 중에만 스크립트 동작). -->
