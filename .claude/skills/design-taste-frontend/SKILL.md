...
description: 새 홍보/랜딩 페이지·포트폴리오·소비자용 웹사이트를 처음부터 만들거나 전면 리디자인할 때 사용(React/Next.js+Tailwind 스택에서 진가 — 새 프로젝트 권장). Anti-slop landing page skill — reads the brief, infers design direction, ships non-templated interfaces. 업무 도구·대시보드·표 중심 관리 화면에는 쓰지 않는다(그건 impeccable 담당). 기존 리뷰웹시스템(workdesk/admin) 화면의 기능 수정·소규모 개선에는 트리거하지 않는다 — 그 경우 CLAUDE.md의 레포 규율이 우선한다.
---

<!-- 설치 메모(2026-08-06): github.com/Leonxlnx/taste-skill 의 skills/taste-skill/SKILL.md 원본(MIT).
     수정 = frontmatter description 만(이 레포에서의 트리거 기준을 새김) — 본문 무수정.
     이미지 생성 도구 강제 규칙(§)은 frontend/docs 시안 문서(오프라인·외부 리소스 0 규율)에는 적용하지 말 것. -->
