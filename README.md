...
### ★ 모집공고 썸네일 URL 즉시 미리보기 (2026-08-15)

모집공고 등록·수정 화면에서 상품 이미지 URL을 붙여넣으면 별도 가져오기 단계 없이 바로 썸네일을 확인할 수 있도록 개선했습니다.

- 쿠팡 CDN 등 HTTPS 이미지 URL을 입력하면 편집 화면에 즉시 미리보기를 표시합니다.
- 입력한 URL을 공고의 썸네일 저장값과 자동으로 동기화해, 저장 후 모집공고 카드에도 같은 이미지가 표시됩니다.
- 이미지 주소가 잘못됐거나 로드에 실패하면 편집 화면에 실패 상태를 안내합니다.
- 테스트 서버에서 Coupang 썸네일 URL로 미리보기와 저장 후 카드 노출까지 확인했습니다.
